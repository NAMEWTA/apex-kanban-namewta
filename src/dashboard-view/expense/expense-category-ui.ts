 ...
 */
export function wireCategorySelect(select: HTMLSelectElement, service: ExpenseService, type: ExpenseType): void {
...
export function showCategoryManager(doc: Document, service: ExpenseService): void {
	...
	function wireSortHandles(rows: HTMLElement[], onReorder: (from: number, to: number) => void): void {
		...
		rows.forEach((row, i) => {
			...
			row.addEventListener('dragover', (e) => {
				...
				clearHints(row);
				row.addClass(
					before ? 'dashboard-expense-catmgr-row--drop-before' : 'dashboard-expense-catmgr-row--drop-after',
				);
			});
	...
	function render(): void {
		...
		for (const type of ['expense', 'income'] as const) {
			...
			addRow(
				section,
				t('expense.cat.namePlaceholder'),
				(name) => announceAdd(service.addCustomCategory(type, name)) !== null,
			);
			...
			primaries.forEach((name, i) => {
				...
				const confirmRemove = async (): Promise<void> => {
					const yes = await showConfirmDialog(null, {
						...
						message: t('expense.cat.primaryRemoveConfirmMessage', {
							name,
							n: service.countPrimaryUsage(type, name),
						}),
					});
			...
			addRow(
				section,
				t('expense.cat.primaryNamePlaceholder'),
				(name) => announceAddPrimary(service.addPrimaryCategory(type, name)) !== null,
			);
		}
	}
	render();
