...
export function showExpenseLedger(doc: Document): void {
	...
	function onKey(e: KeyboardEvent): void {
		...
		if (e.key === 'Escape' && !doc.querySelector('.dashboard-confirm-overlay, .modal-container')) {
			close();
		}
	}
	...
	function rebuildPrimaryFilter(): void {
		...
		const valid =
			primaryFilter === '' || primaryFilter === UNGROUPED_PRIMARY || seen.has(primaryFilter.toLowerCase());
	...
	function filteredRecords(): ExpenseRecord[] {
		...
		const rows = service.getRecords().filter((r) => {
			...
			if (q && !`${r.note ?? ''} ${categoryLabel(r.category)} ${primaryOf(r)}`.toLowerCase().includes(q))
				return false;
			return true;
		});
	...
	function renderTable(): void {
		...
		if (lastFiltered.length === 0) {
			...
			cell.setText(
				t(service.getRecords().length === 0 ? 'expense.ledger.emptyAll' : 'expense.ledger.emptyFiltered'),
			);
		...
		for (const r of pageSlice()) {
			...
			row.createEl('td', {
				cls:
					'dashboard-expense-ledger-td-amount' +
