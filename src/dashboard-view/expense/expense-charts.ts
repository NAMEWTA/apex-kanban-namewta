...
function createChartTip(container: HTMLElement): { show(target: Element, text: string): void; hide(): void } {
	...
	return {
		show(target: Element, text: string): void {
			...
			const left = Math.min(
				Math.max(tRect.left + tRect.width / 2 - cRect.left, 40),
				Math.max(40, cRect.width - 40),
			);
	...
	bars.forEach((b, i) => {
		...
		if (hasSecondary) {
			...
			const rect1 = svg.createSvg('rect', {
				...
				attr: {
					x: slotX + (step - pairW * 2 - 2) / 2,
					y: height - h1,
					width: pairW,
					height: Math.max(b.value > 0 ? 2 : 0, h1),
					rx: 2,
				},
			});
			...
			const rect2 = svg.createSvg('rect', {
				cls: 'dashboard-expense-trend-bar',
				attr: {
					x: slotX + (step - pairW * 2 - 2) / 2 + pairW + 2,
					y: height - h2,
					width: pairW,
					height: Math.max(v2 > 0 ? 2 : 0, h2),
					rx: 2,
				},
			});
...
export function renderExpenseLines(
	...
	for (const [color, label] of [
		...
		const item = legend.createDiv({
			cls: 'dashboard-expense-donut-legend-item dashboard-expense-lines-legend-item',
		});
		const dot = item.createDiv({ cls: 'dashboard-expense-donut-legend-dot' });
