...
export function showExpenseStats(doc: Document): void {
	...
	const kindButtons = HISTORY_KINDS.map((k) =>
		subrangeToggle.createDiv({
			cls:
				'dashboard-expense-subrange-btn' +
				(k.key === historyKind ? ' dashboard-expense-subrange-btn--active' : ''),
			text: t(k.labelKey),
		}),
	);
	...
	const typeButtons = TYPES.map((tp) =>
		typeToggle.createDiv({
	...
	function kpiCard(parent: HTMLElement, value: string, label: string, deltaPct?: number, invert = false): void {
		...
		if (deltaPct !== undefined && Number.isFinite(deltaPct)) {
			...
			const delta = valRow.createDiv({
				cls:
					'dashboard-expense-stats-card-delta' +
	...
	function renderKpis(kpiCol: HTMLElement, win: RangeWindow): void {
		...
		kpiCard(
			row1,
			fmt(totals.expense),
			t('expense.kpiExpenseTotal'),
			deltaOf(totals.expense, prevTotals.expense),
			true,
		);
	...
	function renderMiddleColumn(win: RangeWindow): void {
		...
		for (const lv of [
			...
			const btn = levelToggle.createDiv({
				cls:
					'dashboard-expense-level-btn' +
					(lv.key === categoryLevel ? ' dashboard-expense-level-btn--active' : ''),
				text: t(lv.labelKey),
			});
			btn.addEventListener('click', () => {
		...
		const rows: ExpenseRankRow[] = breakdownSlices(win).map((s) => ({
			key: s.key,
			label: s.label,
			value: s.value,
		}));
	...
	function renderRecords(parent: HTMLElement, win: RangeWindow): void {
		...
		for (const r of records.slice(0, RECORDS_LIMIT)) {
			...
			row.createEl('td', {
				cls:
					'dashboard-expense-records-amount' +
