...
export class QuickNoteConfigModal extends Modal {
	...
	private renderPresetRow(list: HTMLElement, i: number): void {
		...
		this.textInput(
			...
			preset.label,
			'',
			{ cls: 'dashboard-quicknote-cfg-label', placeholder: t('quickNote.fieldLabel') },
			(v) => this.updatePreset(i, { label: v }),
		);
		this.delBtn(top, () => {
		...
		this.pathTextInput(
			card,
			preset.templatePath,
			{ placeholder: t('quickNote.fieldTemplatePh'), mode: 'file' },
			(v) => this.updatePreset(i, { templatePath: v }),
		);
		this.pathTextInput(card, preset.folder, { placeholder: t('quickNote.fieldFolderPh'), mode: 'folder' }, (v) =>
			this.updatePreset(i, { folder: v }),
		);
		this.textInput(card, preset.filename, '', { placeholder: t('quickNote.fieldFilenamePh') }, (v) =>
			this.updatePreset(i, { filename: v }),
		);
	}
	...
	private renderPinnedRow(list: HTMLElement, i: number): void {
		...
		this.textInput(
			...
			'',
			{ cls: 'dashboard-quicknote-cfg-label', placeholder: t('quickNote.fieldLabel') },
			(v) => this.updatePinned(i, { label: v }),
		);
		this.delBtn(top, () => {
			this.pinned = this.pinned.filter((_, idx) => idx !== i);
			this.renderBody();
		});
		this.pathTextInput(card, note.path, { placeholder: t('quickNote.fieldPathPh'), mode: 'file' }, (v) =>
			this.updatePinned(i, { path: v }),
		);
	}
	...
	private renderCommandRow(list: HTMLElement, i: number): void {
		...
		this.textInput(
			...
			'',
			{ cls: 'dashboard-quicknote-cfg-label', placeholder: t('quickNote.fieldLabel') },
			(v) => this.updateCommand(i, { label: v }),
		);
		this.delBtn(top, () => {
	...
	private renderCapture(form: HTMLElement): void {
		...
		this.pathTextInput(
			section,
			this.captureTarget,
			{ placeholder: t('quickNote.fieldCaptureTargetPh'), mode: 'file' },
			(v) => {
				this.captureTarget = v;
			},
		);
		this.pathTextInput(
			section,
			this.captureFolder,
			{ placeholder: t('quickNote.fieldCaptureFolderPh'), mode: 'folder' },
			(v) => {
				this.captureFolder = v;
			},
		);
		this.pathTextInput(
			section,
			this.captureTemplate,
			{ placeholder: t('quickNote.fieldCaptureTemplatePh'), mode: 'file' },
			(v) => {
				this.captureTemplate = v;
			},
		);
		...
		startCb.addEventListener('change', () => {
			if (startCb.checked) {
				this.capturePosition = 'start';
				endCb.checked = false;
			} else startCb.checked = true;
		});
		endCb.addEventListener('change', () => {
			if (endCb.checked) {
				this.capturePosition = 'end';
				startCb.checked = false;
	...
	private indicateDrop(card: HTMLElement, half: 'top' | 'bottom'): void {
		this.clearDragIndicators();
		card.addClass(
			half === 'top' ? 'dashboard-quicknote-cfg-item--drop-before' : 'dashboard-quicknote-cfg-item--drop-after',
		);
	}
	...
	private clearDragIndicators(): void {
		this.contentEl
			...
			.forEach((el) =>
				el.classList.remove(
					'dashboard-quicknote-cfg-item--drop-before',
					'dashboard-quicknote-cfg-item--drop-after',
				),
			);
	}
