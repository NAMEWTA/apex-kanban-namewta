...
import { MomentLike, nowMoment } from '../../shared/datetime';
import { ensureFolder, getOrCreateDailyNote, substituteTemplateVars } from '../calendar/daily-notes';
...
export function renderQuickNoteRegion(
	...
	if (!hasChips && !captureOn) {
		...
		if (dailyOn) {
			chip(nav, 'dashboard-quicknote-chip dashboard-quicknote-today', 'sun', t('quickNote.today'), () =>
				callbacks.onQuickNoteDaily(),
			);
		}
		for (const preset of presets) {
			chip(nav, 'dashboard-quicknote-chip', preset.icon || 'file-plus', preset.label, () =>
				callbacks.onQuickNoteCreate(preset),
			);
		}
		for (const note of pinned) {
			chip(nav, 'dashboard-quicknote-chip dashboard-quicknote-pin', note.icon || 'pin', note.label, () =>
				callbacks.onOpenPinnedNote(note),
			);
		}
		for (const cmd of commands) {
			chip(nav, 'dashboard-quicknote-chip', cmd.icon || 'terminal', cmd.label, () =>
				callbacks.onQuickCommand(cmd),
			);
		}
	}
