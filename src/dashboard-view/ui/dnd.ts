...
export function setupDragAndDrop(
	...
	rows.forEach((colEl, index) => {
		...
		const onDragOver = (e: DragEvent) => {
			...
			if (e.dataTransfer?.types.includes(ITEM_DRAG_TYPE) && !state.sectionDragSource && !state.draggingCardId)
				return;
...
function updateDropIndicator(state: DnDState, column: HTMLElement, clientX: number, clientY: number): void {
	...
	const cards = Array.from(
		cardsContainer.querySelectorAll<HTMLElement>('.dashboard-card:not(.dashboard-card--dragging)'),
	);
...
function updateSectionDropIndicator(
	state: DnDState,
