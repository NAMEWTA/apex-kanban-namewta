...
function rootAnchorIdOf(el: Element): string | null {
	return (
		el.getAttribute('data-card-id') ?? el.getAttribute('data-widget-key') ?? el.getAttribute('data-column') ?? null
	);
}
 ...
 */
function walkScrollers(parent: Element, anchor: string, visit: (el: Element, key: string) => void): void {
