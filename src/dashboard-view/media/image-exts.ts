...
import { renderPagination, renderTagsSelector, folderGroupKey } from '../library';
import { MultiFolderSelectModal } from '../library/folder-config-modal';
import { normalizeExcludeFolders, isUnderExcludedFolder } from '../../shared/exclude-folders';
import { type MediaFileResult, renderMediaGrid, renderMediaList, formatDate } from './media-views';
...
export function renderMediaSection(
	...
	groupSelect.addEventListener('change', () => {
		groupBy = groupSelect.value as MediaGroupMode;
		currentPage = 1;
		render();
	});
	...
	const buildViewToggle = (): void => {
		...
		createToolbarDropdown(
			viewToggle,
			viewMode,
			(['grid', 'list'] as MediaViewMode[]).map((mode) => ({
				key: mode,
	...
	function renderMediaFilterTags(): void {
		...
		for (const tag of filterTags) {
			const chip = filterTagBar.createDiv({
				cls: 'dashboard-library-filter-tag dashboard-library-filter-tag--tag',
			});
		...
		if (filterStart || filterEnd) {
			...
			const end = filterEnd || '...';
			const tag = filterTagBar.createDiv({
				cls: 'dashboard-library-filter-tag',
				text: `${filterProp}: ${start} ~ ${end}`,
			});
			tag.createSpan({ cls: 'dashboard-library-filter-tag-x', text: '×' }).addEventListener('click', () => {
				filterStart = '';
				filterEnd = '';
				refreshMedia();
			});
		...
		for (const folder of filterFolders) {
			...
			const label = tag.createSpan({
				cls: 'dashboard-library-filter-tag-label',
				text: norm.split('/').filter(Boolean).pop() ?? norm,
			});
			label.title = norm;
			tag.createSpan({ cls: 'dashboard-library-filter-tag-x', text: '×' }).addEventListener('click', () => {
	...
	function toggleFilterTag(tag: string, chipsHost: HTMLElement): void {
		filterTags = filterTags.includes(tag) ? filterTags.filter((tg) => tg !== tag) : [...filterTags, tag];
		refreshMedia();
		if (filterPopup) {
			renderTagsSelector(chipsHost, tagService?.getAllTags() ?? [], filterTags, (tg) =>
				toggleFilterTag(tg, chipsHost),
			);
		}
	}
	...
	function openMediaPopup(): void {
		...
		if (tagService) {
			...
			renderTagsSelector(tagChipsHost, tagService.getAllTags(), filterTags, (tag) =>
				toggleFilterTag(tag, tagChipsHost),
			);
		}
		...
		propSelect.addEventListener('change', () => {
			filterProp = propSelect.value as 'created' | 'modified';
			refreshMedia();
		});
		...
		const startInput = dateWrap.createEl('input', {
			cls: 'dashboard-media-filter-date',
			attr: { type: 'date', value: filterStart },
		});
		...
		const endInput = dateWrap.createEl('input', {
			cls: 'dashboard-media-filter-date',
			attr: { type: 'date', value: filterEnd },
		});
		...
		const folderBrowseBtn = folderAddRow.createEl('button', {
			cls: 'dashboard-media-folder-browse',
			text: t('media.browseFolder'),
		});
	...
	filterBtn.addEventListener('click', (e) => {
		e.stopPropagation();
		if (filterPopup) closeMediaPopup();
		else openMediaPopup();
	});
	...
	function render(): void {
		...
		const deleteCb = (f: TFile): void => {
			void deleteWithConfirm(f);
		};
