...
export function appendBacklinks(container: HTMLElement, files: TFile[], onOpenNote?: (file: TFile) => void): void {
	...
	if (files.length > 5) {
		wrap.createDiv({
			cls: 'dashboard-media-backlink dashboard-media-backlink--more',
			text: `+${files.length - 5}`,
		});
	}
}
	...
	for (let i = 0; i < results.length; i++) {
		...
		if (result.tags.length > 0) {
			...
			if (result.tags.length > 2) {
				tagRow.createSpan({
					cls: 'dashboard-media-tile-tag dashboard-media-tile-tag--more',
					text: `+${result.tags.length - 2}`,
				});
			}
		}
