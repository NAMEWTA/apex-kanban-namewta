...
import type { CalendarTaskFilter } from '../calendar/alltasks-scan';
...
export interface DashboardSettings {
	...
	/** One-time intro. Independent of plugin version. */
	introSeen: boolean;
	/** Which products start. Missing keys stay on so older installs keep working. */
	modules: {
		dashboard: boolean;
		editor: boolean;
		terminal: boolean;
	...
	/** Editor side panel layout. Comment text is not stored here. */
	editorWorkbench: EditorWorkbenchSettings;
	/**
	 * Terminal agent settings blob. The terminal product owns the shape.
	 * `null` means it has never been saved; the shell may then import the
	 * previous terminal plugin data once.
	 */
