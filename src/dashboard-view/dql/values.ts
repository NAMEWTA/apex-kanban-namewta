import type { DqlDate, DqlDuration, DqlLink, DqlObject, DqlValue } from './types';
...
export function dqlEquals(a: DqlValue, b: DqlValue): boolean {
	...
	switch (ta) {
		...
		case 'string':
			return a === b;
		case 'date':
			return (a as DqlDate).ts === (b as DqlDate).ts;
		case 'duration':
			return (a as DqlDuration).ms === (b as DqlDuration).ms;
		case 'link':
			return (a as DqlLink).path === (b as DqlLink).path;
		case 'list': {
...
export function dqlCompare(a: DqlValue, b: DqlValue): number | null {
	...
	switch (ta) {
		...
		case 'list':
			return 0; // lists are not orderable; keep stable.
		case 'object':
			return 0;
		default:
			return 0;
...
export function formatValue(v: DqlValue): string {
	...
	switch (kindOf(v)) {
		case 'date':
			return formatDate(v as DqlDate);
		case 'duration':
			return formatDuration(v as DqlDuration);
		case 'link': {
			const link = v as DqlLink;
			return link.display ?? link.path;
		}
		case 'list':
			return (v as DqlValue[]).map(formatValue).join(', ');
		case 'object': {
			const entries = (v as DqlObject).entries;
			return Object.keys(entries)
				.map((k) => `${k}: ${formatValue(entries[k]!)}`)
				.join(', ');
		}
	}
	return '';
