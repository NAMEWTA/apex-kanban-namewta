...
class Parser {
	...
	parse(): Result<Query> {
		...
		if (
			queryType.value === 'HEATMAP' &&
			!heatmapDateField &&
			this.peek().type === 'keyword' &&
			this.peek().text === 'USING'
		) {
			this.advance();
			const dateField = this.parseExpression(0);
			if (!dateField.ok) return dateField;
	...
	private parseQueryType(): Result<QueryType> {
		...
		if (tok.type === 'eof')
			return parseError<QueryType>('Query must start with LIST, TABLE, TASK, CALENDAR, or HEATMAP.');
		return parseError<QueryType>(
			`Expected a query type (LIST/TABLE/TASK/CALENDAR/HEATMAP) but found "${tok.text}".`,
			tok.line,
			tok.column,
		);
	}
	...
	private parseSourceAtom(): Result<FromAtom> {
		...
		return parseError<FromAtom>(
			`Expected a folder, tag, or link in FROM but found "${tok.text}".`,
			tok.line,
			tok.column,
		);
	}
	...
	private keywordLiteral(tok: Token): Expression | null {
		switch (tok.text) {
			...
			case 'TODAY':
				return { type: 'call', name: 'date', args: [{ type: 'literal', value: 'today' }] };
			case 'NOW':
				return { type: 'call', name: 'date', args: [{ type: 'literal', value: 'now' }] };
			case 'TOMORROW':
				return { type: 'call', name: 'date', args: [{ type: 'literal', value: 'tomorrow' }] };
			case 'YESTERDAY':
				return { type: 'call', name: 'date', args: [{ type: 'literal', value: 'yesterday' }] };
			default:
				return null;
		}
	}
