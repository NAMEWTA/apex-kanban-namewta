...
function applyBinOp(
	op: BinOp,
	a: import('./types').DqlValue,
	b: import('./types').DqlValue,
): import('./types').DqlValue {
	switch (op) {
		case '=':
			return dqlEquals(a, b);
		case '!=':
			return !dqlEquals(a, b);
		case '<': {
			const c = dqlCompare(a, b);
...
function addValues(a: import('./types').DqlValue, b: import('./types').DqlValue): import('./types').DqlValue {
	...
	if (ka === 'duration' && kb === 'duration')
		return { kind: 'duration', ms: (a as { ms: number }).ms + (b as { ms: number }).ms };
...
function subtractValues(a: import('./types').DqlValue, b: import('./types').DqlValue): import('./types').DqlValue {
	...
	if (ka === 'date' && kb === 'duration') return makeDate((a as { ts: number }).ts - (b as { ms: number }).ms);
	if (ka === 'date' && kb === 'date')
		return { kind: 'duration', ms: (a as { ts: number }).ts - (b as { ts: number }).ts };
	if (ka === 'duration' && kb === 'duration')
		return { kind: 'duration', ms: (a as { ms: number }).ms - (b as { ms: number }).ms };
	const x = coerceNumber(a);
	const y = coerceNumber(b);
...
function evaluateCall(node: Extract<Expression, { type: 'call' }>, ctx: EvalContext, depth: number): Outcome {
	...
	if (args.length < func.minArgs || args.length > func.maxArgs) {
		const arity =
			func.maxArgs === Number.POSITIVE_INFINITY
		...
		return {
			ok: false,
			error: { stage: 'eval', message: `${func.name}() expects ${arity} argument(s), got ${args.length}.` },
		};
	}
...
function flattenTaskRows(rows: readonly Row[]): Row[] {
	...
	for (const row of rows) {
		...
		for (const task of tasks) {
			...
			const fields =
				kindOf(task) === 'object'
					? (task as { entries: Record<string, import('./types').DqlValue> }).entries
					: {};
...
function matchesAtom(atom: FromClause, page: Page): boolean {
	...
	switch (a.kind) {
		...
		case 'link': {
			...
			return (
				outlinks.some((o) => stripMd(o.toLowerCase()) === bare) ||
				basenameMatch(outlinks, bare) ||
				inlinks.some((o) => stripMd(o.toLowerCase()) === bare) ||
				basenameMatch(inlinks, bare)
			);
		}
		case 'outgoing': {
...
function project(query: Query, rows: readonly Row[], grouped: boolean): QueryResult {
	...
	const columns = query.queryType === 'TABLE' ? buildColumns(query) : [];
	return {
...
function exprLabel(expr: Expression): string {
	switch (expr.type) {
		...
		case 'literal':
			return typeof expr.value === 'string' ? expr.value : coerceString(expr.value ?? '');
		case 'call':
			return expr.name + '()';
		default:
			return 'value';
...
function projectRow(query: Query, row: Row, grouped: boolean): ResultRow {
	...
	if (query.queryType === 'TABLE') {
		...
		const value = query.heatmapValueField
			? evaluate(query.heatmapValueField, ctx, 0)
	...
	const memberRows: ResultRow[] | undefined =
		grouped && row.locals['rows']
