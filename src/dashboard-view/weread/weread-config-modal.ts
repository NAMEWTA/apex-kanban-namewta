	...
	onOpen(): void {
			...
			this.widgets.forEach((w, i) => {
				...
				if (w.view === 'shelf') {
					...
					groupSelect.addEventListener('change', () =>
						update({ groupBy: groupSelect.value as WereadGroupBy }),
					);
					...
					renderFacet(
						panel,
						t('weread.filterContentType'),
						CONTENT_TYPE_OPTIONS,
						w.contentTypeFilters,
						(values) => update({ contentTypeFilters: values }),
					);
					renderFacet(panel, t('weread.filterRecency'), RECENCY_OPTIONS, w.recencyFilters, (values) =>
						update({ recencyFilters: values }),
					);
					renderFacet(panel, t('weread.filterNotes'), NOTE_OPTIONS, w.noteFilters, (values) =>
						update({ noteFilters: values }),
					);
				}
				...
				const upBtn = ops.createEl('button', {
				...
				const downBtn = ops.createEl('button', {
					cls: 'dashboard-weread-cfg-op',
		...
		footer
			...
			.addEventListener('click', () => {
				this.onSave({
					widgets:
						this.widgets.length > 0 ? this.widgets : [{ id: 'w1', view: 'shelf', groupBy: 'readingState' }],
				});
				this.close();
			});
	}
...
function renderStatsItemsPanel(
	...
	const commit = (next: WereadStatItem[]): void => {
		const isDefault =
			next.length === DEFAULT_STAT_ITEMS.length && next.every((item, idx) => item === DEFAULT_STAT_ITEMS[idx]);
...
function appendStatsRowControls(row: HTMLElement, item: WereadStatItem, checked: boolean, onToggle: () => void): void {
