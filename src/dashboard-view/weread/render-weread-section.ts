...
function drawShelf(
	...
	if (totalPages > 1) {
		...
		const prev = pager.createEl('button', {
			cls: 'dashboard-weread-pager-btn',
		...
		prev.addEventListener('click', () => {
			pageState[w.id] = Math.max(1, page - 1);
			drawShelf(content, w, books, pageState);
		});
		next.addEventListener('click', () => {
			pageState[w.id] = Math.min(totalPages, page + 1);
			drawShelf(content, w, books, pageState);
		});
	}
}
...
function renderShelf(content: HTMLElement, books: WereadBookLike[]): void {
	...
	for (const book of books) {
		...
		if (noteTotal > 0)
			meta.createDiv({
				cls: 'dashboard-weread-book-notes',
				text: t('weread.noteBadge', { count: String(noteTotal) }),
			});
...
function renderNotebooks(
	...
	for (const nb of notebooks.slice((page - 1) * PAGE, page * PAGE)) {
		...
		meta.createDiv({
			cls: 'dashboard-weread-notebook-count',
			text: t('weread.noteCount', { n: String(nb.noteCount) }),
		});
		...
		importBtn.addEventListener('click', (e) => {
			e.stopPropagation();
			void doImportHighlights(client, app, importPath, nb, importBtn);
		});
	...
	if (totalPages > 1) {
		...
		next.disabled = page >= totalPages;
		prev.addEventListener('click', () => {
			pageState[widgetId] = Math.max(1, page - 1);
			renderNotebooks(content, client, notebooks, app, importPath, pageState, widgetId);
		});
		next.addEventListener('click', () => {
			pageState[widgetId] = Math.min(totalPages, page + 1);
			renderNotebooks(content, client, notebooks, app, importPath, pageState, widgetId);
		});
	}
}
...
async function doImportHighlights(
	client: WereadClient,
	app: App,
	importPath: string,
...
async function importHighlightsToObsidian(
	app: App,
	importPath: string,
	nb: WereadNotebookLike,
