...
export function filterWereadBooks<T extends WereadFacetedBook>(
	...
	return books.filter((book) => {
		const progressOk =
			!filters.progress?.length || (!!book.readingState && filters.progress.includes(book.readingState));
		const contentOk =
			!filters.contentTypes?.length || (!!book.contentType && filters.contentTypes.includes(book.contentType));
		const recencyOk = !filters.recency?.length || filters.recency.includes(wereadRecency(book, now));
		const notesOk = !filters.notes?.length || filters.notes.some((state) => matchesNoteState(book, state));
...
export function groupWereadBooks<T extends WereadFacetedBook>(
	...
	return order.filter((key) => buckets.has(key)).map((key) => ({ key, books: buckets.get(key) ?? [] }));
}
...
function matchesNoteState(book: WereadFacetedBook, state: WereadNoteState): boolean {
	...
	return (book.noteCount ?? 0) === 0 && (book.bookmarkCount ?? 0) === 0 && (book.reviewCount ?? 0) === 0;
}
