...
import type { WereadContentType, WereadFacetedBook, WereadReadingState } from './weread-shelf-model';
	...
	private async request<T>(apiName: string, params: Record<string, unknown> = {}): Promise<T> {
		...
		for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
			...
			if (data && typeof data['errcode'] === 'number' && data['errcode'] !== 0) {
				...
				throw new Error(
					`API:${typeof errPart === 'string' || typeof errPart === 'number' ? errPart : 'rejected'}`,
				);
			}
			...
			if (upgrade) {
				const msg =
					typeof upgrade === 'object' && upgrade !== null
...
export function parseShelf(data: {
	books?: ShelfBookRaw[];
	albums?: Array<Record<string, unknown>>;
	mp?: Record<string, unknown>;
}): WereadBook[] {
...
export function parseReadStats(data: Record<string, unknown>, mode: WereadStatMode): WereadReadStats {
	...
	const readLongest: WereadTopReadItem[] = Array.isArray(data['readLongest'])
		? (data['readLongest'] as Array<Record<string, unknown>>)
				.map((item) => {
	...
	const preferCategory: WereadPreferCategoryItem[] = Array.isArray(data['preferCategory'])
		? (data['preferCategory'] as Array<Record<string, unknown>>)
				.map((c) => ({
