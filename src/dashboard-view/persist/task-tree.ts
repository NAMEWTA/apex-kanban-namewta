...
export function updateTaskAt(tasks: TaskItem[], path: TaskPath, updater: (t: TaskItem) => TaskItem): TaskItem[] {
...
export function removeTaskAt(tasks: TaskItem[], path: TaskPath): { tasks: TaskItem[]; removed: TaskItem | undefined } {
...
export function insertAt(tasks: TaskItem[], parentPath: TaskPath, index: number, node: TaskItem): TaskItem[] {
	const loc = parentPath.length === 0 ? { siblings: tasks, index: 0 } : locate(tasks, [...parentPath, 0]);
...
export function insertSibling(tasks: TaskItem[], path: TaskPath, node: TaskItem, before: boolean): TaskItem[] {
 ...
 */
export function moveTaskBeside(tasks: TaskItem[], srcPath: TaskPath, destPath: TaskPath, before: boolean): TaskItem[] {
	if (srcPath.length === 0 || destPath.length === 0) return tasks;
	if (isSelfOrDescendant(srcPath, destPath)) return tasks;
