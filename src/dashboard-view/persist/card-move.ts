...
export function memoCardText(card: DashboardCard): string {
	...
	return [
		card.blockquote
			...
			.join('\n'),
		card.body,
		...taskLines(card.tasks),
		...docLines(card.docs),
		card.wikiLink ? `[[${card.wikiLink}]]` : '',
		card.url,
	]
		.filter(Boolean)
		.join('\n');
...
function additionalTasks(card: DashboardCard): TaskItem[] {
	const docs = (nodes: DocNode[]): TaskItem[] =>
		nodes.map((node) => ({
			text: `[[${node.path}]]`,
			checked: false,
...
export function convertMovedCard(card: DashboardCard, source: DashboardColumn, target: DashboardColumn): DashboardCard {
	...
	if (targetType === 'todo' || (targetType === 'sticky' && sourceType === 'todo')) {
		return {
			...moved,
			type: 'task',
			tasks: [...card.tasks, ...additionalTasks(card)],
			body: '',
			blockquote: '',
			docs: [],
			wikiLink: '',
...
export function moveDashboardCard(
	...
	if (
		!source ||
		!target ||
		!['memo', 'todo', 'sticky', 'projects', 'notes', 'dashboard'].includes(sectionType(target))
	)
		return data;
