 ...
 */
function wireCardTouchDrag(body: HTMLElement, card: HTMLElement, index: number, service: HabitService): void {
...
function renderHabitHeatmap(card: HTMLElement, service: HabitService, habit: Habit): void {
	...
	series.forEach((done, i) => {
		...
		cell.setAttribute('title', done > 0 ? `${habitFormatDate(d)} · ${t('habit.heatDone')}` : habitFormatDate(d));
	});
