...
import { calendarSpan, dateBucketOf, taskDayKind, toIsoDate, type VaultTask } from './alltasks-scan';
...
export function renderMonthGrid(
	...
	const wrap = container.createDiv({
		cls: 'dashboard-calendar' + (opts.compact || opts.dotMode ? ' is-compact' : ' is-full'),
	});
...
export function renderWeekGrid(
	...
	const wrap = container.createDiv({
		cls: 'dashboard-calendar dashboard-calendar--week' + (opts.compact ? ' is-compact' : ' is-full'),
	});
...
export function renderWeekTimeGrid(
	...
	for (const { iso } of days) {
		...
		const allDayTasks = (byDay.get(iso) ?? []).filter((task) => !taskDayTime(task, iso)).sort(byDayTaskTime(iso));
		for (const task of allDayTasks.slice(0, 3)) {
			const chip = cell.createDiv({
				cls: 'dashboard-calgrid-allday-chip' + (task.checked ? ' is-done' : ''),
				text: task.text,
			});
			if (opts.onOpenNote) {
	...
	for (const { iso } of days) {
		...
		for (const task of byDay.get(iso) ?? []) {
			...
			if (!opts.compact && opts.onToggle) {
				const check = ev.createEl('input', {
					cls: 'dashboard-calgrid-event-check',
					attr: { type: 'checkbox' },
				});
...
function renderDayTask(task: VaultTask, iso: string, opts: MonthGridOptions): HTMLElement {
	...
	if (multi) {
		const mark = row.createDiv({
			cls: 'dashboard-calendar-multi-mark',
			attr: { 'aria-label': `${task.start} → ${task.end}` },
		});
