...
export class CalendarMonthModal extends Modal {
	...
	/** Vault path of the dashboard file, forwarded when a day agenda opens. */
	...
	private render(): void {
		...
		const filterBtn = header.createEl('button', {
			cls:
				'dashboard-modal-btn dashboard-modal-btn--cancel dashboard-calendar-filter-btn' +
...
export class DayAgendaModal extends Modal {
	...
	private readonly cb: CalendarModalCallbacks;
	/** Vault path of the dashboard file. Still forwarded to insertTaskForDay;
	 *  the write itself always uses the selected day's daily note. */
	private readonly dashboardFile?: string;
	...
	onOpen(): void {
		...
		// Add-task row: optional time (HH:MM) + title + Add. Written into this
		// day's daily note (created from the Daily Notes template when missing).
		...
		titleInput.addEventListener('keydown', (e) => {
			if (e.key === 'Enter') {
				e.preventDefault();
				void this.addTask(titleInput, timeInput);
			}
		});
	...
	private renderRow(task: VaultTask): HTMLElement {
		...
		check.addEventListener('click', (e) => {
			e.preventDefault();
			void this.toggle(task, !task.checked);
		});
		...
		if (task.priority) {
			row.createDiv({
				cls: `dashboard-alltasks-prio dashboard-alltasks-prio--${task.priority}`,
		...
		if (this.cb.onOpenNote) {
			...
			row.addEventListener('click', (e) => {
				...
				if (target.tagName === 'INPUT' || target.closest('a') || target.closest('.dashboard-alltasks-chip'))
					return;
				this.cb.onOpenNote?.(task.file, task.line);
			});
		}
	...
	/** Add the entered task (optional time + title) into this day's daily note.
	 *  The note is created when it does not exist yet. Top or bottom follows
	 *  the calendar widget setting. */
