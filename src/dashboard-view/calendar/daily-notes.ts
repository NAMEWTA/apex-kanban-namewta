...
import { MomentLike, momentOf, nowMoment } from '../../shared/datetime';
 ...
 */
export function substituteTemplateVars(content: string, opts: { title?: string; now?: MomentLike } = {}): string {
...
export interface TaskInsertTarget {
	...
	line: number;
	/** The exact line as written — the optimistic row's originalLine so an
	 *  immediate toggle can match it. */
	writtenLine: string;
	kind: 'daily-top' | 'daily-end' | 'daily-created';
}
 ...
 *
 *  1. a user-pinned file or per-day folder (`customTarget`) replaces the
 *     daily-note path entirely;
 *  2. otherwise the selected day's daily note — reused when it already exists,
 *     created (core Daily Notes folder, format, and template) when it does not.
 *
 * The selected day is the file. A missing note is created for that day; the
 * task is never written into today's journal or the dashboard file.
 *
 * `_dashboardFile` is unused. It stays in the signature so existing call sites
 * that still pass the dashboard path keep compiling.
 *
 * Returns null only when the core Daily Notes plugin is disabled and no
 * custom target is set — callers surface the enable-hint Notice.
 */
export async function insertTaskForDay(
	app: App,
	iso: string,
	taskLine: string,
	_dashboardFile?: string,
	...
	// Selected day's daily note only. Reuse it when present; otherwise create
	// it from the Daily Notes template and insert there. Never today's note.
	...
	const created = await getOrCreateDailyNote(app, iso);
	if (!created) return null;
	return await insertIntoNote(app, created, await app.vault.read(created), taskLine, position);
}
