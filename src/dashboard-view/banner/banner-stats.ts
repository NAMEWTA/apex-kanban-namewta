...
import { momentOf, nowMoment, parseStrict } from '../../shared/datetime';
import { getDailyNotesConfig } from '../calendar/daily-notes';
...
function renderLeftColumn(
	...
	hero.createDiv({
		cls: 'dashboard-banner-stat-label dashboard-banner-stat-label--inline',
		text: t(`banner.stats.${stat}`),
	});
...
function renderCenterColumn(
	...
	if (config.showDetails !== false) {
		...
		const subText = habitSeries ? habitCenterSub(config) : centerSub(stat, r);
