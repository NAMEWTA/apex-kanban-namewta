...
export function createPomodoroMiniPanel(
	...
	function freezeInfoWidth(): void {
		...
		const variants = (['work', 'short-break', 'long-break'] as const).flatMap((phase) => [
			phaseLabel(phase, false),
			phaseLabel(phase, true),
		]);
	...
	function mount(): void {
		...
		const hideBtn = panel.createDiv({
			...
			attr: {
				role: 'button',
				tabindex: '0',
				'aria-label': t('pomodoro.miniHide'),
				title: t('pomodoro.miniHide'),
			},
		});
		setIcon(hideBtn, 'x');
