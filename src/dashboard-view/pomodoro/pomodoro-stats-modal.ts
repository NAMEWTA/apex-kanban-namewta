...
import { PomodoroService, activityColor } from './pomodoro-service';
...
export function showPomodoroStats(doc: Document, service: PomodoroService): void {
	...
	function datesForRange(key: PomodoroRangeKey): {
		curStart: string;
		prevStart: string;
		prevEnd: string;
		dayCount: number;
	...
	function kpiCard(parent: HTMLElement, value: string, label: string, deltaPct?: number): HTMLElement {
		...
		if (deltaPct !== undefined && Number.isFinite(deltaPct)) {
			...
			const delta = valRow.createDiv({
				cls:
					'dashboard-pomodoro-stats-card-delta' +
	...
	const donutContainer = donutSection.createDiv({
		cls: 'dashboard-pomodoro-donut-container dashboard-pomodoro-donut-container--wide',
	});
	...
	function renderDonut(): void {
		...
		const legend = donutContainer.createDiv({
			cls: 'dashboard-pomodoro-donut-legend dashboard-pomodoro-donut-legend--grid',
		});
		for (const [name, mins] of sorted) {
	...
	const trendTitle = trendSection.createDiv({
		cls: 'dashboard-pomodoro-stats-section-title',
		text: t('pomodoro.trend'),
	});
	...
	function renderRanking(): void {
		...
		for (const [name, mins] of sorted) {
			const row = rankContainer.createDiv({
				cls:
					'dashboard-pomodoro-rank-row' +
					(activityFilter === name ? ' dashboard-pomodoro-rank-row--active' : ''),
	...
	function renderTimeline(): void {
		...
		if (records.length === 0) {
			...
			const startBtn = empty.createEl('button', {
				cls: 'dashboard-pomodoro-timeline-start',
		...
		for (const rec of records) {
			...
			if (rec.breakCompleted !== undefined) {
				...
				setIcon(
					sub.createSpan({ cls: 'dashboard-pomodoro-timeline-sub-icon' }),
					rec.breakCompleted ? 'coffee' : 'zap-off',
				);
				sub.createSpan({
				...
				if (rec.interruptions && rec.interruptions > 0) {
					sub.createSpan({
						cls: 'dashboard-pomodoro-timeline-interruptions',
						text: ' · ' + t('pomodoro.interrupted', { count: rec.interruptions }),
					});
				}
			}
		}
