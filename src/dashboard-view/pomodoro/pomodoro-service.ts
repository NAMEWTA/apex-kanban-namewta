	...
	if (raw && typeof raw === 'object') {
		...
		return {
			...
			tags: Array.isArray(obj.tags)
				? obj.tags
						.filter((tg): tg is PomodoroTag => !!tg && typeof tg.name === 'string')
...
export class PomodoroService {
	...
	private getPhaseDurationMs(phase: PomodoroPhase): number {
		...
		switch (phase) {
			case 'work':
				return s.pomodoroWorkMinutes * 60 * 1000;
			case 'short-break':
				return s.pomodoroShortBreakMinutes * 60 * 1000;
			case 'long-break':
				return s.pomodoroLongBreakMinutes * 60 * 1000;
		}
	}
	...
	getState(): PomodoroState {
		const totalSeconds =
			Math.round(this.durationMs / 1000) || Math.round(this.getPhaseDurationMs(this.phase) / 1000);
		return {
			phase: this.phase,
			status: this.status,
	...
	private appendRecord(date: string, record: PomodoroRecord): void {
		...
		const sessions = existing
			? this.data.sessions.map((s) =>
					s.date === date ? { ...s, completed: s.completed + 1, records: [...(s.records ?? []), record] } : s,
				)
	...
	async deleteTag(name: string): Promise<void> {
		this.data = {
			...
			sessions: this.data.sessions.map((s) => ({
				...s,
				records: s.records?.map((r) =>
					r.activity === name ? { ...r, activity: t('pomodoro.defaultActivity') } : r,
				),
			})),
		};
		this.currentActivity = this.data.currentActivity;
	...
	getDailyMinutes(days: number): { date: string; minutes: number }[] {
		...
		for (let i = days - 1; i >= 0; i--) {
			...
			result.push({
				date: dateStr,
				minutes: session ? sessionMinutes(session, this.getSettings().pomodoroWorkMinutes) : 0,
			});
		}
		return result;
	}
	...
	getTodayInterruptions(): number {
		const today = formatDate(new Date());
		return (
			this.sessions.find((s) => s.date === today)?.records?.reduce((sum, r) => sum + (r.interruptions ?? 0), 0) ??
			0
		);
	}
	...
	getStreak(): number {
		const sorted = [...this.sessions].filter((s) => s.completed > 0).sort((a, b) => b.date.localeCompare(a.date));
