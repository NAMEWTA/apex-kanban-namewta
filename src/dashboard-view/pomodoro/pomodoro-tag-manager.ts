...
export function openPomodoroTagManager(doc: Document, service: PomodoroService, onChange: () => void): void {
	...
	function render(): void {
		...
		for (const name of names) {
			...
			const chip = row.createDiv({
				cls:
					'dashboard-pomodoro-tagmanager-chip' +
			...
			chip.addEventListener('click', (e) => {
				...
				list.querySelectorAll('.dashboard-pomodoro-tagmanager-row--open').forEach((r) =>
					r.removeClass('dashboard-pomodoro-tagmanager-row--open'),
				);
			...
			actionBtn(
				actions,
				tag.pinned ? 'pin-off' : 'pin',
				tag.pinned ? t('pomodoro.tagUnpin') : t('pomodoro.tagPin'),
				() => {
					void service.setTagPinned(name, !tag.pinned).then(() => {
						render();
						onChange();
					});
				},
	...
	function dangerBtn(parent: HTMLElement, icon: string, label: string, onClick: () => void): void {
		const btn = parent.createDiv({
			cls: 'dashboard-pomodoro-tagmanager-action dashboard-pomodoro-tagmanager-action--danger',
		});
		setIcon(btn.createSpan({ cls: 'dashboard-pomodoro-tagmanager-action-icon' }), icon);
		btn.createSpan({ cls: 'dashboard-pomodoro-tagmanager-action-label', text: label });
		btn.addEventListener('click', (e) => {
	...
	function showPrompt(
		...
		const cancel = btnRow.createEl('button', {
			cls: 'dashboard-pomodoro-tagmanager-prompt-cancel',
			text: t('common.cancel'),
		});
		cancel.addEventListener('click', closePrompt);
		const ok = btnRow.createEl('button', {
			cls:
				'dashboard-pomodoro-tagmanager-prompt-ok' +
				(options.danger ? ' dashboard-pomodoro-tagmanager-prompt-ok--danger' : ''),
