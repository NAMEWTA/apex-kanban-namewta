...
export function pickNextIndex(length: number, index: number, mode: MusicRepeatMode, userInitiated: boolean): number {
...
export class MusicService {
	...
	async load(): Promise<void> {
		...
		this.index =
			typeof s.musicCurrentIndex === 'number' &&
			s.musicCurrentIndex >= 0 &&
			s.musicCurrentIndex < this.playlist.length
				? s.musicCurrentIndex
				: -1;
	...
	addToPlaylist(tracks: MusicTrack[]): number {
		...
		void backfillCovers(this.playlist)
			.then((withCovers) => {
			...
			.catch(() => {
				/* covers are cosmetic; missing them is not an error */
			});
		return fresh.length;
	}
	...
	async importPlaylist(input: string): Promise<number> {
		...
		try {
			...
			/* keep the imported playlist without covers */
		}
		return fresh.length;
	}
