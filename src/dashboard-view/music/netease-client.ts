...
interface NeteasePlaylistResponse {
	result?: { name?: string; tracks?: NeteaseSong[] };
}
...
export function mapSongUrl(raw: NeteaseSongUrlEntry): SongUrlInfo {
	const url =
		!raw.freeTrialInfo && (raw.code === undefined || raw.code === 200) && typeof raw.url === 'string' && raw.url
			? toHttps(raw.url)
			: null;
	return {
		url,
...
export async function fetchSongUrl(id: number, cookie = '', forceFresh = false): Promise<SongUrlInfo> {
	...
	const resp = await requestUrl({
		url,
		headers: cookie ? { ...NETEASE_HEADERS, Cookie: cookie } : NETEASE_HEADERS,
		throw: false,
	});
