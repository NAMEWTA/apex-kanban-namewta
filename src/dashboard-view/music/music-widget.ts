...
export function renderSidebarMusicWidget(
	...
	bg?: import('../types').WidgetBackground,
	...
	volBtn.addEventListener('click', (e) => {
		...
		volRange = volArea.createEl('input', {
			attr: {
				...
				min: '0',
				max: '100',
				value: String(Math.round((svc?.getState().volume ?? 0.8) * 100)),
			},
		});
...
function renderPlaylistRows(refs: MusicWidgetRefs, state: MusicPlayerState | null): void {
	...
	state.playlist.forEach((track, i) => {
		const row = refs.panel.createDiv({
			cls:
				'dashboard-sidebar-music-row' +
				(i === state.currentIndex ? ' dashboard-sidebar-music-row--active' : '') +
