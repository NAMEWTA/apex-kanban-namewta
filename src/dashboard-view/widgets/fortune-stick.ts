...
export function drawFortuneStick(category: FortuneCategory): FortuneStick {
	...
	const seed = today.getFullYear() * 10000 + (today.getMonth() + 1) * 100 + today.getDate() + hashCategory(category);
	...
	const availableSticks = sticks.length > 0 ? sticks : FORTUNE_STICKS.filter((s) => s.category === category);
