...
const LAYER_STATE_CLASSES = [
	...
	...ALBUM_TRANSITIONS.flatMap((m) => [
		TRANSITION_CLASSES[m].start,
		...(TRANSITION_CLASSES[m].out ? [TRANSITION_CLASSES[m].out] : []),
	]),
];
...
export function renderSidebarAlbumWidget(container: HTMLElement, settings: DashboardSettings, app: App): void {
	...
	const renderFrame = (): void => {
		...
		const frame = body.createDiv({
			cls: `dashboard-sidebar-album-frame dashboard-sidebar-album-frame--${transition}`,
		});
