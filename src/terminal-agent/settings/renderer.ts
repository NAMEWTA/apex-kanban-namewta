...
import type { BinaryDownloadSource, PresetScript, ShellType } from './model';
...
import { BaseSettingsRenderer } from './base-renderer';
import { t } from '../i18n';
import { PresetScriptModal } from '../view/preset-script-modal';
import { renderPresetScriptIcon } from '../view/preset-script-icons';
import { getSelectableShellTypes } from '../terminal/shell-profiles';
import {
  getAiLauncherEntry,
...
} from '../terminal/ai-launcher-catalog';
import {
  readinessToBadge,
  type AiLauncherStatusSnapshot,
} from '../terminal/ai-launcher-status';
import { clearCommandVersionCache } from '../terminal/command-version-probe';
...
export class TerminalSettingsRenderer extends BaseSettingsRenderer {
  ...
  private readonly builtInPresetIds = new Set(['claude-code', 'codex', 'opencode', 'grok']);
  /**
  ...
  renderSection(
    ...
    section: 'shell' | 'instance' | 'workflows' | 'appearance' | 'behavior' | 'connection' | 'visibility',
  ): void {
    ...
    if (section === 'shell') this.renderShellSettings(containerEl);
    else if (section === 'instance') this.renderInstanceBehaviorSettings(containerEl);
    else if (section === 'workflows') this.renderPresetScriptsSettings(containerEl);
    else if (section === 'appearance') this.renderDisplaySettings(containerEl);
    else if (section === 'behavior') this.renderBehaviorSettings(containerEl);
    else if (section === 'connection') this.renderServerConnectionSettings(containerEl);
    else this.renderVisibilitySettings(containerEl);
  }
   ...
   * label and version info everywhere NAND lists their AI launchers.
   *
  ...
  private renderThemePreview(container: HTMLElement): void {
    ...
    this.themePreviewContentEl.createDiv({ text: '$ echo "NAND"' });
    this.themePreviewContentEl.createDiv({ text: 'NAND' });
