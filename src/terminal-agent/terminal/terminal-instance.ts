...
import { getHomeDir, getPlatform, isWindows } from '../platform';
import { t } from '../i18n';
import type { ServerManager } from '../server/server-manager';
import type { PtyClient } from '../server/pty-client';
import type { PtyConfig, ShellEvent, ShellEventSource } from '../server/types';
import type { DefaultShellOption } from './terminal-service';
import { EnhancedKeyboardProtocol, formatPastedTerminalText } from './enhanced-keyboard-protocol';
import {
  createSynchronizedOutputCompatibilityState,
  filterSynchronizedOutputScrollbackPurge,
} from './ai-tui-compatibility';
import {
...
export class TerminalInstance {
  ...
  private setupContextMenu(container: HTMLElement): void {
    this.addDomEventListener(container, 'contextmenu', (e: MouseEvent) => {
      ...
      // Either way NAND stops the host (browser / Obsidian) from also
      ...
      if (action === 'suppress') {
        ...
        // xterm.js mouse tracking. NAND must not also paste here, otherwise
  ...
  private findPromptLine(direction: 'previous' | 'next'): number | null {
    ...
    if (direction === 'previous') {
      for (let index = validPromptLines.length - 1; index >= 0; index -= 1) {
        const line = validPromptLines[index];
        if (line !== undefined && line < currentViewportLine) {
          return line;
        }
      }
  ...
  private convertWslPathToWindows(wslPath: string): string {
    ...
    if (wslMountMatch?.[1] !== undefined && wslMountMatch[2] !== undefined) {
   ...
   *
   * NAND invokes this only with directory paths (from `getCwd()`),
