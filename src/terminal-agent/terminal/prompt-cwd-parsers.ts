 ...
 *
 * The implementation is intentionally regex-based because NAND
...
export function extractPowerShellCwd(cleanData: string): string | null {
  ...
  const captured = matches[matches.length - 1]?.[1];
  return captured?.trimEnd() || null;
}
...
export function extractGitBashWindowTitleCwd(rawData: string): string | null {
  ...
  return convertGitBashPath(captured);
}
...
function convertGitBashPath(rawPath: string, userProfileDir = ''): string {
  if (/^\/[a-zA-Z]\//.test(rawPath)) {
    const drive = rawPath[1];
    if (!drive) return rawPath;
