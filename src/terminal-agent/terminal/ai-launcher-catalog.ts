 ...
 *
 * NAND currently only ships "coding agent" launchers (Claude Code, Codex,
 ...
 */
import { AGENT_CATALOG } from '../launch/catalog';
...
/**
 * Where NAND looks up the CLI's latest version. The lookup itself is opt-in
...
export interface AiLauncherCatalogEntry {
   ...
   * NAND never executes these — the user has to paste them into a shell
   ...
   * {@link installCommands} these are intended to be run by NAND itself
   * inside a user-visible terminal session — the user clicks "Update now"
   * in the install modal and NAND types the command into a fresh
   ...
   * NAND is automating a step the user could type by hand, with full
...
export const AI_LAUNCHER_CATALOG: readonly AiLauncherCatalogEntry[] = AGENT_CATALOG.map((agent) => ({
  presetId: agent.id,
  category: 'coding-agent' as const,
  detectCommand: agent.detectCommand,
  installDocsUrl: agent.installDocsUrl,
  installCommands: agent.installCommands,
  upgradeCommands: agent.upgradeCommands,
  versionRegistry: agent.versionRegistry,
}));
