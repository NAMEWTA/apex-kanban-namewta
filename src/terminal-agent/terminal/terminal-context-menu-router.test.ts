...
import { resolveTerminalContextMenuAction } from './terminal-context-menu-router.ts';

test('plain shells keep showing the NAND context menu on right-click', () => {
  assert.equal(
...
test('Claude Code sessions yield right-click to the TUI without pasting from NAND', () => {
  // Claude Code already pastes on right-click through xterm.js mouse tracking.
  // NAND must NOT also call pasteFromClipboard, otherwise the user sees the
...
test('Shift+RightClick is a universal escape hatch back to the NAND menu', () => {
