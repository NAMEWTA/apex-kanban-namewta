...
import type { ServerManager } from '../server/server-manager';
import type { ClaudeCodeIdeBridge } from '../bridge/claude-ide-bridge';
import type { AgentContextBridge } from '../bridge/agent-context-bridge';
import { TERMINAL_VIEW_TYPE, TerminalView } from '../view/terminal-view';
...
} from '../terminal/always-on-top-terminal-display';
import { getLeafForTerminalRoute, avoidOccupiedTerminalLeaf } from '../terminal/terminal-leaf-routing';
import {
  AI_LAUNCHER_CATALOG,
  getAiLauncherEntry,
...
} from '../terminal/command-availability';
import { clearCommandVersionCache, probeCommandVersion } from '../terminal/command-version-probe';
import { clearLatestVersionCache, fetchLatestVersion } from '../terminal/latest-version-registry';
import {
...
export class TerminalAgentController {
  ...
  async getServerManager(): Promise<ServerManager> {
    if (!this._serverManager) {
      ...
      const { ServerManager } = await import('../server/server-manager');
  ...
  async getTerminalService(): Promise<TerminalService> {
    ...
    if (!this._terminalService) {
      ...
      const { TerminalService } = await import('../terminal/terminal-service');
  ...
  async onload() {
    ...
    // Set debug mode
    const { setDebugMode } = await import('../logger');
  ...
  private async initializeClaudeCodeIdeBridge(): Promise<void> {
    if (!this._claudeCodeIdeBridge) {
      const { ClaudeCodeIdeBridge } = await import('../bridge/claude-ide-bridge');
  ...
  private async initializeAgentContextBridge(): Promise<void> {
    if (!this._agentContextBridge) {
      const { AgentContextBridge } = await import('../bridge/agent-context-bridge');
  ...
  private createLauncherStatusBadge(status: AiLauncherStatus, dot = false): HTMLElement {
    ...
    badge.className = dot
      ? 'preset-scripts-menu-status-badge is-dot'
