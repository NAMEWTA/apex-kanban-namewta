...
import { t } from '../i18n';
import { PRESET_SCRIPT_ICON_OPTIONS, renderPresetScriptIcon } from './preset-script-icons';
import {
  listObsidianCommandOptions,
  searchObsidianCommandOptions,
  type ObsidianCommandOption,
} from './obsidian-command-search';
...
export class PresetScriptModal extends Modal {
  ...
  private attachCommandSuggestions(
    ...
    valueInput.addEventListener('keydown', (event) => {
      ...
      if (event.key === 'Enter' && suggestionPanel.hasClass('is-open') && selected) {
        event.preventDefault();
