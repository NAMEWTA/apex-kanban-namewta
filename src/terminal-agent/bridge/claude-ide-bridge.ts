...
const IDE_TOOLS = [
  {
    ...
    description: 'Close all Claude Code diff tabs in the editor. In NAND this is a no-op.',
  ...
  {
    name: 'getDiagnostics',
    description: 'Get editor diagnostics. Obsidian/NAND currently returns an empty diagnostics list.',
    inputSchema: {
      type: 'object',
      properties: {
        uri: {
...
export class ClaudeCodeIdeBridge {
  ...
  private decodeBufferChunks(chunks: Buffer[]): string {
    ...
    for (const chunk of chunks) {
      for (let index = 0; index < chunk.byteLength; index++) {
        bytes[offset + index] = chunk[index] ?? 0;
      }
  ...
  private handleInitialize(
    ...
    this.sendResult(socket, id, {
      ...
      instructions:
        'This NAND bridge exposes the active Obsidian file and selection to compatible agent CLIs.',
    });
  }
