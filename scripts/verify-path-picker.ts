...
interface MockFolder extends TFolder {
	path: string;
	name: string;
}
...
const folder = (path: string): MockFolder => Object.assign(new TFolder(), { path, name: path.split('/').pop()! });
...
async function main(): Promise<void> {
	...
	// 1. File mode: lists files (not folders), each row showing basename + parent.
	{
		...
		const names = findByClass(modal.contentEl as unknown as El, 'dashboard-pathpicker-name').map(
			(e) => e.textContent,
		);
		assert.ok(names.includes('daily'), '1: basename label');
		const parents = findByClass(modal.contentEl as unknown as El, 'dashboard-pathpicker-parent').map(
			(e) => e.textContent,
		);
	...
	// 3. Typing filters: substring on basename or path, case-insensitive.
	{
		...
		assert.equal(
			findByClass(modal.contentEl as unknown as El, 'dashboard-pathpicker-name')[0]!.textContent,
			'weekly',
		);
		...
		assert.ok(
			findByClass(modal.contentEl as unknown as El, 'dashboard-library-empty').length === 1,
			'3: empty state shown',
		);
	}
	...
	// 4. Clicking a row delivers the full path and closes the modal.
	{
		let picked = '';
		let closed = 0;
		const modal = new PathPickerModal(makeApp(FILES, FOLDERS), 'folder', (p) => {
			picked = p;
		});
		modal.onOpen();
	...
	{
		let picked = '';
		const modal = new PathPickerModal(makeApp(FILES, FOLDERS), 'file', (p) => {
			picked = p;
		});
		modal.onOpen();
		const rows = findByClass(modal.contentEl as unknown as El, 'dashboard-pathpicker-item');
	...
	//    input and reports it through onPick.
	{
		...
		const seen: string[] = [];
		const btn = attachPathPicker(parent as unknown as HTMLElement, input, makeApp(FILES, FOLDERS), 'file', (p) => {
			seen.push(p);
		});
