 ...
 *
 * Run: `pnpm run test:countdown-picker`
 */
...
const dayButtons = (): El[] =>
	findByClass(popup, 'dashboard-task-reminder-calendar-day').filter(
		(b) => !b.hasClass('dashboard-task-reminder-calendar-day--other-month'),
	);
const ymCells = (): El[] => findByClass(popup, 'dashboard-countdown-ym-cell');
const navArrows = (): El[] =>
	findTag(findByClass(popup, 'dashboard-task-reminder-calendar-nav')[0]!, 'button').filter(
		(b) => !b.hasClass('dashboard-countdown-cal-label'),
	);
...
assert.ok(
	dayButtons().some((b) => b.textContent === '15' && b.hasClass('dashboard-task-reminder-calendar-day--selected')),
	'stored day selected',
);
...
assert.ok(
	ymCells().some((c) => c.textContent === '9月' && c.hasClass('dashboard-task-reminder-calendar-day--selected')),
...
findTag(footer, 'button')
	...
	.click();
assert.equal(
	(saved as { targetDate?: string } | undefined)?.targetDate,
	'2025-04-20T14:45',
