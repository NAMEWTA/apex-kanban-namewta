...
const tagSection = findByClass(content, 'dashboard-library-config-section').find(
	(section) =>
		findByClass(section, 'dashboard-media-folder-input-row').length > 0 &&
		findByClass(section, 'dashboard-library-filter-values').length > 0,
);
assert.ok(tagSection, 'tag filter section found');

const searchInput = findTag(tagSection, 'input').find((el) => el.getAttribute('placeholder') === '搜索标签…');
...
const emptyText = (): string | null => findByClass(chipsHost, 'dashboard-library-filter-empty')[0]?.textContent ?? null;
...
assert.ok(
	orderIndex(tagSection, searchInput!) < orderIndex(tagSection, chipsHost),
...
const savingContent = openModal([], (r) => {
	saved = r;
});
