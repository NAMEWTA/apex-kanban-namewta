 ...
 *
 * Run: `pnpm run test:library-group`
 */
...
assert.deepEqual(
	propGroups.map((g) => g.label),
	['done', 'todo', '(未设置)'],
	'scalar buckets in first-occurrence order, not-set last',
);
...
assert.deepEqual(
	arrayGroups.map((g) => g.label),
	['x', 'y'],
	'array value fans out',
);
...
const numericGroups = groupLibraryResults([mk('notes/a10/1.md', {}), mk('notes/a2/2.md', {})], 'folder', undefined, [
	'notes',
]);
assert.deepEqual(
	numericGroups.map((g) => g.label),
	['a2', 'a10'],
	'numeric-aware alphabetical folder order',
);
...
renderLibrarySection(
	...
	(cfg) => {
		savedCount++;
		saved = { ...cfg } as Record<string, unknown>;
	},
);
...
assert.ok(
	!findByClass(el, 'dashboard-library-page-size')[0]!.hasClass('is-hidden'),
	'page-size select shown ungrouped',
);
...
assert.deepEqual(
	headers().map((h) => findByClass(h, 'dashboard-library-group-name')[0]!.textContent),
	['a', 'b'],
	'group names',
);
assert.deepEqual(
	headers().map((h) => findByClass(h, 'dashboard-library-group-count')[0]!.textContent),
	['2', '1'],
	'group counts',
);
assert.equal(bodies().length, 2, 'one body per group');
assert.ok(
	bodies().every((b) => findByClass(b, 'dashboard-library-grid').length === 1),
	'each body holds its own grid',
);
assert.ok(
	findByClass(el, 'dashboard-library-page-size')[0]!.hasClass('is-hidden'),
	'page-size select hidden while grouped',
);
assert.equal(
	findByClass(el, 'dashboard-library-pagination')[0]!.childElementCount,
	0,
	'no pagination controls while grouped',
);
...
assert.deepEqual(
	headers().map((h) => findByClass(h, 'dashboard-library-group-name')[0]!.textContent),
	['done', 'a', '(未设置)'],
	'property groups inherit the section sort',
);
...
assert.ok(
	!findByClass(el2, 'dashboard-library-page-size')[0]!.hasClass('is-hidden'),
	'page-size select shown in the degraded state',
);
