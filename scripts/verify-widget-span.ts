...
const run = (): void => {
	...
	// --- 1. resolveStackedSpans: the packing rules -------------------------
	{
		...
		assert.deepEqual(
			spansFor(['weather', 'album-1'], { albums: [albumCfg(1, 'full')] }),
			[4, 2],
			'album downshifts to fill',
		);
		...
		assert.deepEqual(
			spansFor(['reading', 'lunar', 'countdown-x']),
			[3, 2, 2],
			'countdown starts a new column after a 1-row hole',
		);
		assert.deepEqual(
			spansFor(['reading', 'lunar', 'habit']),
			[3, 2, 4],
			'tiered card also refuses a 1-row remainder (fresh column, preferred kept)',
		);
	...
	// --- 2. buildStackedSpanSpecs: tiers, prefixes, defaults ---------------
	{
		...
		assert.deepEqual(
			specs.map((s) => s.preferred),
			[4, 3, 2, 6, 2, 2, 4, 6],
			'habit defaults two-thirds, reading half, album per-instance (legacy -> full), prefixes 2, fixed table, unknown 6',
		);
		assert.deepEqual(
			specs.map((s) => s.allowed ?? null),
			[ALL_TIERS, ALL_TIERS, ALL_TIERS, ALL_TIERS, null, null, null, null],
...

	{
		...
		assert.equal(
			cssVarOf(byKey('reading'), '--db-widget-span'),
			'3',
			'reading keeps its preferred in the fresh column',
		);
	...
	// --- 4. Signature: ratios in, sizing out -------------------------------
	{
		const sig = (over: Partial<DashboardSettings>): string =>
			sidebarWidgetSignature(
				baseSettings({ widgetHabitEnabled: true, readingEnabled: true, ...over }),
				false,
				true,
				false,
				'',
