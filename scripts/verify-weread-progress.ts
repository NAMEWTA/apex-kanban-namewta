...
assert.equal(
	progressFromGetProgress({ book: { progress: 45 }, progress: 99 }),
	45,
	'nested beats flat when both exist',
);
...
for (const state of ['notStarted', 'reading', 'finished'] as const) {
	assert.ok(
		shelf.some((b) => b.readingState === state),
		`shelf produces a ${state} example`,
	);
}
...
assert.equal(
	facetShelf.find((b) => b.bookId === 'reading')?.lastReadTime,
	now - 2 * 86400_000,
	'seconds timestamp -> milliseconds',
);
...
const filtered = filterWereadBooks(
	faceted,
	{
		progress: ['reading'],
		contentTypes: ['book'],
...
assert.deepEqual(
	filtered.map((b) => b.bookId),
	['reading'],
	'four facets combine with AND',
);

const readingGroups = groupWereadBooks(faceted, 'readingState', now);
assert.deepEqual(
	readingGroups.map((g) => g.key),
	['reading', 'finished', 'notStarted'],
	'reading groups use product order',
);
const contentGroups = groupWereadBooks(faceted, 'contentType', now);
assert.deepEqual(
	contentGroups.map((g) => g.key),
	['book', 'audio', 'article'],
	'content groups separate shelf item types',
);
assert.deepEqual(
...
const merged = mergeProgressFiles(
	{
		version: 1,
		entries: {
			a: { progress: 10, readingState: 'reading', ts: 100 },
			b: { progress: 20, readingState: 'reading', ts: 500 },
		},
	},
	{
		version: 1,
		entries: {
			a: { progress: 15, readingState: 'reading', ts: 200 },
			c: { progress: 30, readingState: 'reading', ts: 300 },
		},
	},
);
...
assert.ok(
	storeLike.entries['fresh'] && nowMs - storeLike.entries['fresh']!.ts < PROGRESS_FRESH_MS,
	'recent entry is fresh',
);
assert.ok(
	storeLike.entries['stale'] && nowMs - storeLike.entries['stale']!.ts >= PROGRESS_FRESH_MS,
	'aged entry is stale',
);
