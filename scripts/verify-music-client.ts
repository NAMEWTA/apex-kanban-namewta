...
ok(
	mapSongUrl({ url: 'https://m.music.126.net/a.mp3', code: 200, freeTrialInfo: { start: 0, end: 30 } }).url === null,
	'trial clips must not count as full member playback',
);
...
ok(
	...
	'only auth cookies are forwarded',
);
...
ok(
	detail !== null && detail.picUrl === 'http://p3.music.126.net/cover.jpg',
	'picUrl kept when present (later upgraded via toHttps at play time)',
);
...
ok(
	mapSearchSong({ id: 5, name: 'x', artists: [{ name: '' }] })?.artist === '',
	'blank artist names filtered, no dangling " / "',
);
...
ok(
	!isPlayableByFee(2) && !isPlayableByFee(4) && !isPlayableByFee(undefined),
	'album-purchase/unknown fees not playable',
);
...
ok(
	repeated.length === 2 && repeated[0]?.timeMs === 62500 && repeated[1]?.timeMs === 100500,
	'one line two tags -> two entries (62500/100500ms)',
);
const mixed = lines.filter((l) => l.text === '两种精度混排');
ok(mixed[0]?.timeMs === 90000, 'mm:ss without fraction works');
ok(
	lines.every((l, i) => i === 0 || (lines[i - 1]?.timeMs ?? 0) <= l.timeMs),
...
ok(
	extractPlaylistId('https://music.163.com/playlist?id=123456&userid=7') === '123456',
	'query-form link, first id wins',
);
