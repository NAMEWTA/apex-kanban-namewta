 ...
 *
 * Run: `pnpm run test:visible-properties`
 */
...
assert.deepEqual(selectBadgeKeys(fm, [], 2), ['title', '作者'], 'auto mode respects the cap');
...
const chipState = (): Array<[string, boolean]> =>
	findByClass(host, 'dashboard-library-filter-chip').map(
		(c) => [c.textContent, c.hasClass('active')] as [string, boolean],
	);
...
assert.ok(
	chipState().some(([k, on]) => k === '出版社' && on),
	'initial pick rendered active',
);

// Toggling picks in click order: 作者 then 类型.
findByClass(host, 'dashboard-library-filter-chip')
	.find((c) => c.textContent === '作者')!
	.click();
findByClass(host, 'dashboard-library-filter-chip')
	.find((c) => c.textContent === '类型')!
	.click();
...
const addBtn = findTag(host, 'button').find((b) => findByClass(host, 'dashboard-modal-btn--confirm').includes(b))!;
addInput.value = '我的字段';
addBtn.click();
...
const modal = new FolderConfigModal(app, [], [], [], undefined, undefined, undefined, (r) => {
	saved = r;
});
modal.onOpen();
...
const pickerChips = findByClass(content, 'dashboard-library-filter-values').find((v) =>
	findByClass(v, 'dashboard-library-filter-chip').some((c) => c.textContent === '作者'),
)!;
findByClass(pickerChips, 'dashboard-library-filter-chip')
	.find((c) => c.textContent === '作者')!
	.click();
const footer = findByClass(content, 'dashboard-modal-footer')[0]!;
findTag(footer, 'button')
	.find((b) => b.hasClass('dashboard-modal-btn--confirm'))!
	.click();
assert.deepEqual(
	(saved as { visibleProperties?: string[] } | undefined)?.visibleProperties,
	['作者'],
...
const modal2 = new FolderConfigModal(app, [], [], [], undefined, undefined, undefined, (r) => {
	saved2 = r;
});
modal2.onOpen();
const footer2 = findByClass(modal2.contentEl as unknown as El, 'dashboard-modal-footer')[0]!;
findTag(footer2, 'button')
	.find((b) => b.hasClass('dashboard-modal-btn--confirm'))!
	.click();
assert.strictEqual(
	(saved2 as { visibleProperties?: string[] } | undefined)?.visibleProperties,
	undefined,
