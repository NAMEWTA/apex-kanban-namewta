 ...
 *
 * Run: `pnpm run test:media-thumb-size`
 */
...
const activeSize = (el: El): string | undefined =>
	findByClass(el, 'dashboard-media-size-toggle')[0]
		? findByClass(findByClass(el, 'dashboard-media-size-toggle')[0]!, 'dashboard-toolbar-dropdown-text')[0]
				?.textContent
