...
async function main(): Promise<void> {
	...
	assert.ok(
		events.some((e) => e.textContent.includes('Alpha')),
		'1: vault task shown',
	);
	assert.ok(
		events.some((e) => e.textContent.includes('Beta')),
		'1: timed task shown',
	);
	...
	assert.ok(
		!findByClass(host, 'dashboard-calendar-filter-btn')[0]!.hasClass('is-filtered'),
		'1: filter defaults to all',
	);
	...
	const betaCheck = findByClass(body, 'dashboard-calendar-check').find((c) =>
		c.parent ? c.parent.textContent.includes('Beta') : false,
	)!;
	assert.ok(betaCheck, '2: checkbox rendered for full-mode task');
	betaCheck.click();
	await flush();
	assert.ok(
		(store.get('notes/timed.md') ?? '').includes(`- [x] Beta ⏰ ${todayIso} 09:30`),
		'2: file updated to checked',
	);
	const body2 = findByClass(host, 'dashboard-calendar-section-body')[0]!;
	assert.ok(
		findByClass(body2, 'dashboard-calendar-event').some((e) => e.hasClass('is-done')),
		'2: row re-rendered as done',
	);
	...
	assert.ok(
		findByClass(body3, 'dashboard-calgrid-event').some((e) => e.textContent.includes('Beta')),
		'3: timed task positioned in grid',
	);
	...
	assert.ok(
		findByClass(body4, 'dashboard-calendar-event').some((e) => e.textContent.includes('Gamma')),
		'4: new task appears after rescan',
	);
	assert.equal(
		findByClass(host, 'dashboard-calendar-nav-label')[0]!.textContent,
		labelAfterPrev,
		'4: navigation preserved across refresh',
	);
	...
	//    grid re-render (navigation).
	{
		const lines = Array.from(
			{ length: 12 },
			(_, i) => `- [ ] Task${String(i + 1).padStart(2, '0')} ⏰ ${todayIso} 09:${String(i).padStart(2, '0')}`,
		);
		...
		await flush();
		assert.equal(
			findByClass(previewBody, 'dashboard-calendar-day-preview').length,
			0,
			'6: navigation closes stale popup',
		);
	}
