...
const buildReorderedTree = (): El => {
	...
	const widgetsEl = find(
		find(find(regionEl, 'dashboard-sidebar'), 'dashboard-sidebar-scroll'),
...
const widgets2 = find(
	find(find(region2, 'dashboard-sidebar'), 'dashboard-sidebar-scroll'),
	'dashboard-sidebar-widgets',
);
...
const widgets3 = find(
	find(
		find(find(find(foreign, 'dashboard-main'), 'dashboard-scroll-region'), 'dashboard-sidebar'),
...
assert.equal(
	scrollOf(find(find(foreign, 'dashboard-main'), 'dashboard-scroll-region')).top,
	321,
	'other scrollers still restored',
);
