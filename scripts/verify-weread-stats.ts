 ...
 *
 * Run: `pnpm run test:weread-stats`
 */
...
assert.deepEqual(
	parseReadStats({ readTimes: [1, 2], readStat: 'junk' }, 'monthly').readTimes,
	[],
	'array readTimes / junk shapes degrade to empty',
);
...
assert.deepEqual(
	computeReadStreaks(
		buckets.map((b) => (b.ts === day(9) ? { ...b, seconds: 0 } : b)),
		now,
	),
	{ current: 3, longest: 4 },
...
assert.deepEqual(
	...
	[
		{ ts: 1757059200 * 1000, seconds: 600 },
		{ ts: 1757145600 * 1000, seconds: 90 },
	],
...
assert.equal(
	buildHeatmapCells([], nowNoon).filter((c) => c.inWindow).length,
	HEATMAP_WINDOW_DAYS,
	'no buckets still yields the full window at level 0',
);

// Shelf state overlay: finished wins, persisted entry refines, reading time
// marks reading, otherwise not started.
assert.equal(
	shelfStateFor({ readingState: 'finished' }, { readingState: 'reading' }),
	'finished',
	'shelf finished verdict survives the entry',
);
assert.equal(
	shelfStateFor({ readingState: 'reading' }, { readingState: 'notStarted' }),
	'notStarted',
	'entry refines a non-finished shelf state',
);
assert.equal(
	shelfStateFor({ readingState: 'reading', readingTime: 3600 }),
	'reading',
	'no entry + reading time -> reading',
);
assert.equal(
	shelfStateFor({ readingState: 'reading', readingTime: 0 }),
	'notStarted',
	'no entry + no reading time -> not started',
);
assert.equal(
	shelfStateFor({ readingState: 'notStarted', readingTime: 120 }, { readingState: 'reading' }),
	'reading',
	'entry upgrades a shelf notStarted',
);
...
assert.deepEqual(
	normalizeStatItems(['kpi', 'preferCategory']),
	['kpi', 'preferCategory'],
	'stored preferCategory stays renderable',
);
assert.deepEqual(normalizeStatItems(['kpi', 'bogus' as WereadStatItem]), ['kpi'], 'unknown ids drop out');
assert.deepEqual(
	normalizeStatItems(['bogus' as WereadStatItem]),
	['kpi', 'topRead', 'trend'],
	'all-unknown -> default',
);
