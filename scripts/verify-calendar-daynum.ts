...
function isTrigger(el: El): boolean {
	return el.hasClass('is-clickable') && el.getAttribute('role') === 'button' && el.getAttribute('tabindex') === '0';
}
...
async function main(): Promise<void> {
	...
	// 5. Week time grid: the 7 day headers are triggers reporting Mon..Sun.
	{
		...
		assert.equal(
			seen[6],
			toIsoDate(new Date(monday.getFullYear(), monday.getMonth(), monday.getDate() + 6)),
			'5: last header is Sunday',
		);
	}
	...
	{
		const root = new El('div');
		renderWeekTimeGrid(root as unknown as HTMLElement, mondayOf(new Date()), new Map(), {
			compact: false,
			app: APP,
		});
