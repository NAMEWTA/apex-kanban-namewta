...
const run = (): void => {
	...
	assert.equal(
		isStackedLayout(baseSettings({ layoutMode: 'stacked' })),
		true,
		'stacked setting is stacked (non-phone)',
	);
	...
	// 2. Side build: flat structure, saved order rules.
	{
		...
		assert.deepEqual(
			...
			'side build follows the saved order as direct children',
		);
	}
	...
	const dragCase = (mode: 'side' | 'stacked', coords: { clientX: number; clientY: number }): string => {
		...
		const beforeOrder =
			mode === 'stacked' ? ['yearProgress', 'lunar', 'quickActions'] : ['quickActions', 'lunar', 'yearProgress'];
		const afterOrder =
			mode === 'stacked' ? ['lunar', 'yearProgress', 'quickActions'] : ['lunar', 'quickActions', 'yearProgress'];
		assert.deepEqual(
			orders[0],
			indicator === 'top' ? beforeOrder : afterOrder,
	...
	// clientX must be ignored (a stray X-axis branch would flip these).
	{
		...
		assert.equal(
			dragCase('stacked', bottomCoords),
			'bottom',
			'stacked: below midpoint -> bottom (clientX ignored)',
		);
