...
assert.deepEqual(docPaths(parsed), [PLAIN_PDF], 'plain doc path parses');

const bracketMd = baseMd + `\n- [[${BRACKET_PDF}]]`;
const bracketParsed = parse(bracketMd);
assert.deepEqual(docPaths(bracketParsed), [PLAIN_PDF, BRACKET_PDF], 'bracketed path parses with the full name intact');

// ---------- 2. round-trip ----------
const once = parse(serialize(bracketParsed));
assert.deepEqual(docPaths(once), [PLAIN_PDF, BRACKET_PDF], 'bracketed doc survives serialize -> parse');
const twice = parse(serialize(once));
assert.deepEqual(docPaths(twice), [PLAIN_PDF, BRACKET_PDF], 'round-trip is stable on the second pass');
...
// ---------- 4. nested + collapsed ----------
const nestedMd =
	baseMd + '\n' + [`- [[父目录/[英] Guide.pdf]] <!--collapsed-->`, `    - [[${BRACKET_PDF}]]`].join('\n');
...
assert.ok(!docPaths(doubleLink).some((p) => p === 'a' || p === 'b'), 'one-line double wikilink never becomes a doc');
assert.deepEqual(docPaths(parse(serialize(parsed))), [PLAIN_PDF], 'plain doc path unaffected by the regex change');
