 ...
 *
 * Run: `pnpm run test:music-account`
 */
...
async function run(): Promise<void> {
	...
	const track = (id: number, name: string): MusicTrack => ({
		...
		durationMs: 1000,
		fee: 1,
		picUrl: `https://p.music.126.net/${id}.jpg`,
	});
	...
	assert.equal(
		stopMsgs.filter((m) => m.includes('dead1') || m.includes('dead2')).length,
		2,
		'both dead tracks named',
	);
	assert.ok(
		stopMsgs.some((m) => /已停止|Stopped/.test(m)),
		`circuit-breaker notice shown: ${stopMsgs.join(' | ')}`,
	);
	...
	assert.ok(
		messages()
			.slice(seen)
			.some((m) => m.includes('dead1')),
		'anonymous fee-1 click keeps the vipSkipped notice',
	);
	assert.equal(calls.length, 0, 'no network for a pre-flight skip');

	process.stdout.write(
		'verify-music-account: login, cancellation, cleanup, account-isolated playback and -110 retry/skip notices OK\n',
	);
}
