 ...
 *
 * Run: `pnpm run test:album-widget`
 */
...
const albumSettings = (over: Partial<DashboardSettings>): DashboardSettings =>
	({
		widgetAlbumEnabled: true,
		widgetAlbumFolder: '',
		widgetAlbumIntervalSec: 8,
		widgetAlbumRecursive: true,
		widgetAlbumRatio: '1:1' as '1:1' | '3:4',
		widgetAlbumTransition: 'fade' as 'fade' | 'slide-left' | 'slide-right' | 'zoom',
		...over,
...
const run = async (): Promise<void> => {
	...
	// 1. Unset folder -> "pick a folder" placeholder.
	{
		...
		renderSidebarAlbumWidget(
			...
			albumSettings({ widgetAlbumFolder: '' }),
			makeVault([]).app,
		);
	...
	// 2. Folder set, no images -> "no images" placeholder.
	{
		...
		renderSidebarAlbumWidget(
			root as unknown as HTMLElement,
			albumSettings({ widgetAlbumFolder: 'Empty' }),
			makeVault(['Empty/readme.md']).app,
		);
		const widget = findByClass(root, 'dashboard-sidebar-album')[0]!;
		const ph = findByClass(widget, 'dashboard-sidebar-album-placeholder')[0]!;
	...
	// 3. listAlbumImages semantics.
	{
		...
		assert.deepEqual(
			listAlbumImages(app, 'Photos', true),
			['Photos/2024/sub/s.png', 'Photos/a2.png', 'Photos/a10.png', 'Photos/b.jpg'],
			'recursive: natural sort, hidden/other folders/md skipped',
		);
		assert.deepEqual(
			listAlbumImages(app, 'Photos', false),
			['Photos/a2.png', 'Photos/a10.png', 'Photos/b.jpg'],
	...
	// 4. Populated folder: frame + img + badge; single image -> --single, no timer.
	{
		...
		renderSidebarAlbumWidget(
			root as unknown as HTMLElement,
			albumSettings({ widgetAlbumFolder: 'Photos' }),
			makeVault(['Photos/a2.png', 'Photos/a10.png']).app,
		);
		const widget = findByClass(root, 'dashboard-sidebar-album')[0]!;
		...
		assert.equal(
			layers.every((l) => l.getAttribute('draggable') === 'false'),
			true,
			'layers not natively draggable',
		);
	...
	}
	{
		...
		renderSidebarAlbumWidget(
			root as unknown as HTMLElement,
			albumSettings({ widgetAlbumFolder: 'Solo' }),
			makeVault(['Solo/only.png']).app,
		);
		const widget = findByClass(root, 'dashboard-sidebar-album')[0]!;
	...
	// 4b. Portrait ratio setting applies the 3:4 modifier class.
	{
		...
		renderSidebarAlbumWidget(
			root as unknown as HTMLElement,
			albumSettings({ widgetAlbumFolder: 'Photos', widgetAlbumRatio: '3:4' }),
			makeVault(['Photos/a2.png']).app,
		);
		const widget = findByClass(root, 'dashboard-sidebar-album')[0]!;
	...
	//     the old one out, and settles with flipped roles + advanced badge.
	{
		...
		renderSidebarAlbumWidget(
			root as unknown as HTMLElement,
			albumSettings({ widgetAlbumFolder: 'Photos', widgetAlbumTransition: 'slide-left' }),
			makeVault(['Photos/a2.png', 'Photos/a10.png']).app,
		);
		const widget = findByClass(root, 'dashboard-sidebar-album')[0]!;
		const frame = findByClass(widget, 'dashboard-sidebar-album-frame')[0]!;
	...
	// 7. Timer teardown: destroyAlbumWidgets clears; detached tick self-cleans.
	{
		...
		renderSidebarAlbumWidget(
			detachedRoot as unknown as HTMLElement,
			albumSettings({ widgetAlbumFolder: 'Photos' }),
			app,
		);
...
run().then(
	() => console.log('album widget: ALL PASS'),
	(e) => {
		console.error(e);
		process.exit(1);
	},
