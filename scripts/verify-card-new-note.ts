...
function main(): void {
	...
	//    modal); memo sections keep their original header buttons.
	{
		...
		).addEventListener('dashboard-library-config', (ev) => {
		...
		const memo = renderSection(makeColumn('memo', [makeCard()]), callbacks, app) as unknown as El;
		assert.ok(
			!findTag(memo, 'button').some((b) => b.getAttribute('aria-label') === '分区设置'),
			'5: memo section has no gear',
		);
	}
	...
	//    unset config means vault root (empty string).
	{
		assert.equal(
			sectionNewNoteFolder({
				...
				sortBy: 'modified',
				sortDesc: true,
				folders: [' /Projects/Sub/ '],
			}),
			'Projects/Sub',
			'6: configured folder trimmed',
		);
		assert.equal(
			sectionNewNoteFolder({ filters: [], viewMode: 'grid', sortBy: 'modified', sortDesc: true, folders: [] }),
			'',
			'6: empty folders = vault root',
		);
	...
	//    (trimmed) inputs — the browse pickers write into the same inputs.
	{
		...
		const saveBtn =
			findTag(content, 'button').filter((b) => b.textContent === '保存')[0] ??
