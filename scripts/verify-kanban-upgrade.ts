...
const badgeTexts = (host: El): string[] => findByClass(host, 'dashboard-library-badge').map((b) => b.textContent);

// Group-by field drops from badges (it IS the column); other properties badge.
assert.deepEqual(
	badgeTexts(renderKanban({})),
	['titlex'],
...
assert.deepEqual(
	badgeTexts(renderKanban({ visibleProperties: ['status', 'title'] })),
	['titlex'],
...
assert.equal(
	findByClass(renderKanban({ propertyLimit: 0 }), 'dashboard-library-badge').length,
	0,
...
assert.equal(
	findByClass(renderKanban({ showProperties: false }), 'dashboard-library-badge').length,
	0,
	'showProperties false hides kanban badges',
);
...
// in property mode).
assert.deepEqual(
	badgeTexts(renderKanban({ groupMode: 'folder', folders: ['notes'] })),
	['statustodo', 'titlex'],
